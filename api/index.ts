...
async function sendOTPEmail(email: string, code: string) {
  ...
  try {
    ...
    throw new Error("Falha ao tentar enviar o código de acesso por e-mail.");
  }
}
...
app.post("/api/auth/verify-otp", async (req, res) => {
  ...
  if (
    user &&
    !user.isAdmin &&
    (email === "mazzulli.danilo@gmail.com" || (await prisma.user.count()) === 1)
  ) {
...
app.delete(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(400)
        .json({ error: "Este usuário possui registros vinculados." });
    }
  },
);
...
app.delete("/api/products/:id", authenticateToken, async (req, res) => {
  try {
    ...
    res.status(400).json({
      error: "Este produto possui dependências e não pode ser excluído.",
    });
  }
});
...
app.post("/api/sales", authenticateToken, async (req, res) => {
  ...
  console.log(
    `Processando venda: Data ${new Date(date)}, Quantidade ${quantity}, Total ${product.price * quantity}`,
  );
...
app.put("/api/reservations/:id", authenticateToken, async (req, res) => {
  ...
  if (!existing)
    return res.status(404).json({ error: "Ops! Reserva não encontrada" });
